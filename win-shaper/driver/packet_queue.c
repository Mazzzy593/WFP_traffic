...
void ProcessQueue(PACKET_QUEUE *queue) {
  ...
  do {
    ...
    if (!IsListEmpty(&queue->bandwidth_queue)) { 
      ...
      // CONTAINING_RECORD ???????????QUEUED_PACKET??????????????搇istEntry???????
...
/*-----------------------------------------------------------------------------
  MAC frame Test 打印MAC帧的地址
-----------------------------------------------------------------------------*/
BOOLEAN zzpMACFrameTest(_Inout_opt_ void* layerData,
    BOOLEAN outbound,
...
/*-----------------------------------------------------------------------------
  ...
  ?????Driver???????????????????????????????????
-----------------------------------------------------------------------------*/
BOOLEAN ShaperQueuePacket(_In_ const FWPS_INCOMING_VALUES* inFixedValues,   // 经过包装的数据包的相关信息， 有layer ID, source and destination addresses
    _In_ const FWPS_INCOMING_METADATA_VALUES* inMetaValues,   // 经过包装的数据信息，interface index, timestamps
    _Inout_opt_ void* layerData,
    BOOLEAN outbound, // 标志位
    _In_ HANDLE injection_handle)
{
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "this is ShaperQueuePacket\n"));
    BOOLEAN queued = FALSE;

    PACKET_QUEUE* queue = outbound ? &outbound_queue : &inbound_queue;
    LARGE_INTEGER now = KeQueryPerformanceCounter(NULL);

    // see if we need to drop the packet because of plr   丢包率筛查
    if (traffic_shaping_enabled && queue->plr > 0) {
        ...
        if (random < queue->plr)
            return TRUE;      // 这里是按照丢包率丢弃了数据包，属于正常处理返回True
    }

    // 通过设置的参数，将buff中的MAC帧分段或者填充
    if (FWPS_IS_L2_METADATA_FIELD_PRESENT(inMetaValues, FWPS_L2_METADATA_FIELD_ETHERNET_MAC_HEADER_SIZE))
    {
        zzpMACFrameTest(layerData, outbound, injection_handle);
        //MACFrameTestFlag = FALSE;
    }

    // see if we need to drop the packet because the buffer is "full"  缓冲区筛查
    NET_BUFFER_LIST* nbl = (NET_BUFFER_LIST*)layerData;
    ...
    if (traffic_shaping_enabled && layerData) {
        ...
        if (packet) {
            ...
            if (!outbound) {
                ...
                if (FWPS_IS_L2_METADATA_FIELD_PRESENT(inMetaValues, FWPS_L2_METADATA_FIELD_ETHERNET_MAC_HEADER_SIZE))
                    // 检查所需的帧头部信息是否在inMetaValues中存在，因为inMetaValues信息是不固定的结构体
            ...
            // Clone the buffer 复制缓冲区
            packet->packet_length = data_length;
            packet->packet_count = packet_count; // 经过计算的当前缓冲区的所有数据包个数和缓冲区长度
       
            if (bytesRetreated) // 如果meta控制信息中有帧头部信息，这里才会做检查然后clone buffer
            ...
            if (NT_SUCCESS(status)) {
                ...
                InsertTailList(&queue->bandwidth_queue, &packet->listEntry);    // 把packet的listEntry，入队bandwidth_queue，从这里开始才有联系
                KeReleaseInStackQueuedSpinLock(&lock);
                packet = NULL;
                queued = TRUE;

                // Start the timer to process the packet queue if it isn't already running
                ProcessQueue(queue);        // 将数据包入队之后，开始ProcessQueue
                StartPacketTimerIfNecessary();
            }
        }
    }
