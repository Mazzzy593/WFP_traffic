...
typedef struct {
  unsigned __int32 self_max;
  unsigned __int32 self_min;
  unsigned short   self_rate;
  BOOLEAN          enable_flag;
} SELF_PARAMS;
...
#define	SHAPER_IOCTL_IPF CTL_CODE(FILE_DEVICE_NETWORK, 0x805, METHOD_BUFFERED, FILE_WRITE_ACCESS)   // IPF = IPfragment
#define	SHAPER_IOCTL_TCPF CTL_CODE(FILE_DEVICE_NETWORK, 0x806, METHOD_BUFFERED, FILE_WRITE_ACCESS)  // TCPF = TCPfragment
#define	SHAPER_IOCTL_MACP CTL_CODE(FILE_DEVICE_NETWORK, 0x807, METHOD_BUFFERED, FILE_WRITE_ACCESS)  // MACP = MACpadding
